  ...
  var canvas = {
                ...
                case "text":
                  ...
                  if (data.splitLine == false) {
                    ...
                    splitedLines.forEach(function (line, lineIndex){
                      ...
                      ctx.fillText(line, data.x, data.y + ((data.fontSize + data.addExtraTextPixels * lineIndex) * index), data.textMax)
                    })
                  }
                  ctx.stroke();
                  ctx.fill();
    ...
    app: {
      ...
      createGradient: function(x1, y1, x2, y2, colorStop) {
        ...
        rt.gradient = grad
        rt.imageUrl = canvas.app.entityToImg(new canvas.entity({ fill: rt.gradient, width: x2, height: y2, cropWidth: x2, cropHeight: y2, render: false }, false))
        rt.linearImage = canvas.app.entityToImg(new canvas.entity({ fill: rt.gradient, width: x2, height: 1, cropWidth: x2, cropHeight: 1, render: false }, false))
