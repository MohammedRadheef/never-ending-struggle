function Mango() {
  var canvas = {
        ...
        if (layer.entities) {
          ...
          layer.entities.forEach(function(data, dataIndex) {
            ...
            if (data.render == true) {
              ...
              ctx.save();
              canvas.toRender(ctx, data);
              data.savedTransform = ctx.getTransform();
              //ctx.restore();
              
              // var pt = new Path2D()
              // pt.rect(data.x, data.y, data.getWidth(), data.getHeight())
              // ctx.clip(pt)

              data.childrens.forEach((childEntity, childIndex) => {
                childEntity = (childEntity.data ? childEntity.data : childEntity);

                //ctx.save();
                var ct = new OffscreenCanvas(100,100)
                ct.getContext('2d')
                canvas.toRender(ctx, childEntity);
                //ctx.restore();
              });
              ctx.restore();
    ...
    entity: function(data, addOn = true) {
      var tempData = new canvas.EntityData();
